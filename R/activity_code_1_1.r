...
#' @param index Which of the three indices returned by overlap::overlapEst to use.
...
#' @export
compareCkern <- function(y1,y2,reps=1000,index=c("Dhat4","Dhat5","Dhat1"))
{ index <- match.arg(index)
  i <- switch(index,
         "Dhat1"=1,
         "Dhat4"=2,
         "Dhat5"=3)
   ovl <- overlap::overlapEst(y1,y2)[i]
   names(ovl) <- NULL
   samp <- matrix(sample(c(y1,y2), reps*length(c(y1,y2)), replace=TRUE), ncol=reps)
   res <- pbapply::pbsapply(1:reps, function(j)
     overlap::overlapEst(samp[1:length(y1),j], samp[length(y1)+(1:length(y2)),j])[i])
   fun <- ecdf(res)
...
#' @param dat Character string defining whether to plot the data distribution and if so which style to use.
...
#' @param ... Additional arguments passed to internal plot call affecting only axes.
#' @docType methods
#' @rdname plot-actmod-methods
#' @export
setMethod("plot", "actmod",
          function(x, xunit=c("hours","radians"), yunit=c("frequency","density"), dat=c("histogram","rug","none"), add=FALSE, dcol=1, lcol=2, ...){

            #setup function sets up the plot with appropriate scales and labels with flexibility for midifcation using ...
            setup <- function(x,y,xunit,yunit){
              if(xunit=="hours"){
                Hours <- x
                if(yunit=="frequency"){
                  Frequency<-y
                  if("ylim" %in% names(list(...))){
                    if("xaxp" %in% names(list(...)))
                      plot(Hours,Frequency, type="n", ...) else
                        plot(Hours,Frequency, type="n", xaxp=c(0,24,4), ...)
                  }else{
                    if("xaxp" %in% names(list(...)))
                      plot(Hours,Frequency, type="n", ylim=c(0,max(y,d,ucl)), ...) else
                        plot(Hours,Frequency, type="n", xaxp=c(0,24,4), ylim=c(0,max(y,d,ucl)), ...)
                  }
                }else{
                  Density<-y
                  if("ylim" %in% names(list(...))){
                    if("xaxp" %in% names(list(...)))
                      plot(Hours,Density, type="n", ...) else
                        plot(Hours,Density, type="n", xaxp=c(0,24,4), ...)
                  }else{
                    if("xaxp" %in% names(list(...)))
                      plot(Hours,Density, type="n", ylim=c(0,max(y,d,ucl)), ...) else
                        plot(Hours,Density, type="n", xaxp=c(0,24,4), ylim=c(0,max(y,d,ucl)), ...)
                  }
                }
              }else{
                Radians <- x
                if(yunit=="frequency"){
                  Frequency<-y
                  if("ylim" %in% names(list(...)))
                    plot(Radians,Frequency, type="n", ...) else
                      plot(Radians,Frequency, type="n", ylim=c(0,max(y,d,ucl)), ...)
                }else{
                  Density<-y
                  if("ylim" %in% names(list(...)))
                    plot(Radians,Density, type="n", ...) else
                      plot(Radians,Density, type="n", ylim=c(0,max(y,d,ucl)), ...)
                }
              }
            }
            ...
            if(!add)
            {	setup(range(x), c(0,max(y,d,ucl)), xunit, yunit)
              if(dat=="histogram")
                lines(h$breaks, c(d,d[1]), type="s", col=dcol) else
              if(dat=="rug")
                for(i in 1:length(data))
                  lines(rep(data[i],2),max(y)*-c(0,0.03), lwd=0.1, col=dcol)
            }
            lines(x, y, col=lcol)
            if(length(lcl)>0)
